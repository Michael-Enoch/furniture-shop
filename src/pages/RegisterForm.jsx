...
const Register = () => {
  const { userData, register: signUp } = useAuth();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  useEffect(() => {
    if (userData && registerSuccess) {
      toast.success(
        `Welcome to Hudson Furnishings ${userData.name || "Guest"}`,
        {
          style: {
            backgroundColor: "#3A2F2A",
            color: "#F8F5F2",
  ...
  }, [userData, registerSuccess, navigate]);
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <FaChair size={23} style={{ color: theme.colors.accent.DEFAULT }} />
          </motion.div>
          <h2
